...
# Proxy - Eğer kullanmıyorsanız None yapın
proxies = None  # Proxy kullanmıyorsanız None
# proxies = {
...
class Connection:
    def __init__(self, username, tc, avater, roomID, agent, throttle, msg_cb, loop):
        ...
        self.set_loglevel(logging.INFO)  # INFO seviyesinde başlat
    ...
    async def write(self, dir, content, ext='txt'):
        current_date = datetime.datetime.now().strftime('%Y-%m-%d')  # İngilizce format
    ...
    async def get_session(self):
        if self.session is None:
            self.session = AsyncSession(headers=headers, cookies=self.cookies, proxies=proxies, impersonate="chrome")
        return self.session
    ...
    async def get_login_token(self):
        ...
        if stat == 200:
            ...
            if 'token' in resp_parsed:
                token = resp_parsed['token']
                self.debug('token值：' + token)
                return token
            else:
                self.error(f'Response does not contain token: {resp_parsed}')
                return None
        else:
    ...
    async def post_login(self, token):
        ...
        self.debug('登录返回值：' + str(resp.status_code))
    ...
    async def login(self):
        ...
        if token is not None:
            stat, resp = await self.post_login(token)
            ...
            if stat == 200:
                ...
                self.error(f"登录失败: {resp_parsed.get('error', 'Unknown error')}")
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if 'application/json' in content_type:
                    ...
                    self.error(f"更新房间信息失败1: {resp_json.get('error', 'Unknown error')}")
                    return
            ...
            try:
                ...
                if 'application/json' in content_type:
                    try:
                        ...
                        self.error(f"JSON decode error in join_room: {resp.text[:200]}")
                        return
                    ...
                    try:
                        ...
                        self.error(f"进入房间失败 - Non-JSON response: {resp.text[:200]}")
                        return
                        
                    if 'error' in resp_json:
                        self.error(f"进入房间失败1: {resp_json.get('error', 'Unknown error')}")
                        return
            ...
            try:
                ...
                if 'application/json' in content_type:
                    try:
                        ...
                        self.debug(f"JSON decode error. Response: {resp.text[:200]}")
                        await asyncio.sleep(1)
                        continue
                else:
                    try:
                        resp_json = json.loads(resp.text)
                        if 'error' in resp_json:
                            self.debug(f"更新房间信息失败: {resp_json.get('error', 'Unknown error')}")
                            self.room_connected = False
                            break
                    except json.JSONDecodeError:
                        self.debug(f"Non-JSON response: {resp.text[:200]}")
                        await asyncio.sleep(1)
                        continue
                ...
                if stat == 200:
                    if resp_parsed is None:
                        continue  # JSON parse başarısız, döngüye devam
                                ...
                                for msg in [x for x in msgs if x is not None]:
                                    ...
                                    if msg.message and msg.message != 'keep':
                                        ...
                                        if user and type(user) is not str:
                                            ...
                                            userInfo = str(user) if user else 'Unknown'
    ...
    async def send_post(self, data):
        for _ in range(self.retries):
            try:
                ...
                if resp.status_code != 200:
                    self.error('发送【' + data.get('message', '') + '】失败')
                return
            except Exception as e:
                self.error('发送【' + data.get('message', '') + '】时产生错误' + str(e))
                self.error(traceback.format_exc())
                await asyncio.sleep(1)
